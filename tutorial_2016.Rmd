...
A matrix $\mathbf{B}$ oficialmente é a matriz residual quando a variação entro de grupos e excluida da variação total. Para fazer essa conta podemos usar novamente a função lm() e obter a soma dos residuos ao quadrado (uma espécie de medida da variação total de todos os individuos em relação à média). Fazemos isso tanto para a variação entre grupos quanto dentro de grupos:
...
Não é identico, mas tá quase lá. Com mais espécies essas duas estimativas ficam cada vez mais similares.
